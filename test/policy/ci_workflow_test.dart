...
void main() {
  ...
  group('ci workflow', () {
    test('every runner is a pinned macOS label', () {
      final runners = RegExp(r'runs-on:\s*(\S+)')
          .allMatches(workflow)
          .map((m) => m.group(1)!)
          .toSet();
      ...
      for (final runner in runners) {
        expect(
          ...
          isTrue,
          reason: 'the app ships on iOS only; a Linux job would go green over '
              'a platform nobody ships. Found: $runner',
        );
        expect(
          runner,
          isNot('macos-latest'),
          reason: 'ci-pipeline-and-gates rule 2: image drift moves Xcode and '
    ...
    test('the Flutter action is pinned to the same version as the record', () {
      ...
      final pinned = RegExp(r"flutter-version:\s*'([^']+)'")
          .firstMatch(workflow)
          ?.group(1);
      final recorded = RegExp(r'"flutter":\s*"([^"]+)"')
          .firstMatch(File('.toolchain.json').readAsStringSync())!
          .group(1);

      expect(
        pinned,
        recorded,
        reason: '.fvmrc is deliberately not used, so flutter-action cannot '
    ...
    test('every required gate step is present', () {
      final missing =
          kRequiredSteps.where((s) => !workflow.contains(s)).toList();
    ...
    test('no step can pass without passing, and no gate blesses', () {
      ...
      expect(
        ...
        isFalse,
        reason: 'ci-pipeline-and-gates rule 9: a gate verifies, it never '
    ...
    test('codegen runs before analyze', () {
      expect(
        ...
        reason: 'codegen-and-toolchain rule 1: analyzing before generating '
  ...
  group('pull request template', () {
    final template = File('.github/PULL_REQUEST_TEMPLATE.md').readAsStringSync();

    test('carries the five required sections', () {
      final missing =
          kRequiredPrSections.where((s) => !template.contains(s)).toList();
    ...
    test('its verification checklist quotes the gate runner', () {
      expect(
        ...
        reason: 'the runner is the only sanctioned way to run the skill gates, '
